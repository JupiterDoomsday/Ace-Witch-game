...
namespace Yarn.Unity
{
    public class CustomLineView : DialogueViewBase
    {
        ...
        /// </summary>
        CustomYarnEffects.CoroutineInterruptToken currentCustomStopToken = new CustomYarnEffects.CoroutineInterruptToken();
        ...
        private void DismissLineInternal(Action onDismissalComplete)
        {
            ...
            currentCustomStopToken.Complete();
        ...
        private IEnumerator RunLineInternal(LocalizedLine dialogueLine, Action onDialogueLineFinished) 
        {
            IEnumerator PresentLine()
            {
                ...
                if (useTypewriterEffect)
                {
                    ...
                    yield return StartCoroutine(CustomYarnEffects.CustomizableTypewriter(
                        lineText,
                        typewriterEffectSpeed,
                        () => onCharacterTyped.Invoke(),
                        () => onPauseStarted.Invoke(),
                        () => onPauseEnded.Invoke(),
                        () => onTextDoneTyping.Invoke(),
                        pauses,
                        currentCustomStopToken
                    ));

                    if (currentCustomStopToken.WasInterrupted)
                    {
        ...
        public override void UserRequestedViewAdvancement()
        {
            ...
            if (currentCustomStopToken.CanInterrupt)
            {
                // Stop the current animation, and skip to the end of whatever
                // started it.
                currentCustomStopToken.Interrupt();
            }
            else
            {
